  ...
  <div class="overflow-x-auto rounded-lg shadow-inner border border-gray-200">
    <table class="min-w-full table-auto border-collapse">
      <thead class="bg-gradient-to-r from-purple-600 to-indigo-600 text-white uppercase text-xs tracking-wider">
        <tr>
          <th class="px-5 py-3 text-left cursor-pointer" @click="sortBy('UserId')">User Name<SortIcon :active="sortKey === 'UserId'" :asc="sortAsc" /></th>
          ...
          <th class="px-5 py-3 text-center cursor-pointer" @click="sortBy('type')"> Seat <SortIcon :active="sortKey === 'seat'" :asc="sortAsc" /></th>
          <th class="px-5 py-3 text-center cursor-pointer" @click="sortBy('date')">Date <SortIcon :active="sortKey === 'date'" :asc="sortAsc" /></th>
          <th class="px-5 py-3 text-left cursor-pointer" @click="sortBy('phone')">Phone<SortIcon :active="sortKey === 'phone'" :asc="sortAsc" /></th>
      ...
      <transition-group tag="tbody" name="fade" class="divide-y divide-gray-200">
        ...
        >
          ...
          <td class="px-5 py-3 text-center">{{ Array.isArray(booking.seatNumbers) ? booking.seatNumbers.join(', ') : booking.seatNumbers || '—' }}</td>
          <td class="px-5 py-3 text-center">{{ formatDate(booking.date) }}</td>
          <td class="px-5 py-3 text-left">{{ booking.phone }}</td>
...
const completedUsers = ref([]); // store completed bookings
const showCompleted = ref(false); // toggle completed view

async function deleteBooking(id) {
  if (!confirm('Are you sure you want to complete this booking?')) return;

  try {
    await axios.put(`http://localhost:5000/api/bookings/${id}/complete`);
