<template>
  <div class="font-khmer min-h-screen flex items-center justify-center px-4 py-12 ">
    <div class="flex w-full max-w-4xl bg-white rounded-xl shadow-2xl overflow-hidden  h-[80vh]">
      ...
      <div class="w-1/2 bg-gray-100 p-8 flex items-center justify-center relative overflow-hidden">
        <img src="@/assets/img/login-bg.jpg" alt="Login Background"
          class="absolute inset-0 w-full h-full object-cover opacity-60 transition-opacity duration-500 hover:opacity-80" />

        <div class="relative z-10 text-center bg-black bg-opacity-40 p-6 rounded-lg">
          <h1
            class="text-4xl font-bold bg-gradient-to-r from-orange-500 via-yellow-500 to-red-500 bg-clip-text text-transparent drop-shadow-lg">
            {{ $t('log.TitleApp') }}
          </h1>
          <p
            class="text-lg font-medium bg-gradient-to-r from-orange-400 via-pink-500 to-purple-500 bg-clip-text text-transparent mt-2 drop-shadow-lg">
            {{ $t('log.Welcome') }}
          </p>
        </div>
      </div>
      ...
      <div class="w-1/2 p-10 flex flex-col justify-center">
        <h2
          class="text-2xl font-bold text-gray-800 mb-4 bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">
          {{ $t('log.TitleSignIn') }}
        </h2>

        <form @submit.prevent="handleLogin" class="space-y-6">
          <div class="relative">
            <label class="block text-sm font-medium text-gray-700">{{ $t('log.Email') }}</label>
          ...
          <div class="relative">
            <label class="block text-sm font-medium text-gray-700">{{ $t('log.Password') }}</label>
          ...
          <button type="submit" :disabled="loading"
            ...
            {{ loading ? 'Signing In...' : $t('log.SignIn') }}
          </button>

          <p class="mt-4 text-sm text-center text-gray-600">
            {{ $t('log.DoNotHaveAcc') }}
            <router-link to="/register"
              class="text-purple-600 font-medium hover:underline transition-colors duration-300">
              {{ $t('log.Register') }}
            </router-link>
          </p>
