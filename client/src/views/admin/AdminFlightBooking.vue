    ...
    <div class="overflow-x-auto rounded-lg shadow-inner border border-gray-200">
      <table class="min-w-full table-auto border-collapse">
        <thead class="bg-gradient-to-r from-purple-600 to-indigo-600 text-white uppercase text-xs tracking-wider">
            <tr>
                <th class="px-5 py-3 text-left cursor-pointer" @click="sortBy('UserId')">
                    User ID
                    <SortIcon :active="sortKey === 'UserId'" :asc="sortAsc" />
                </th>
                <th class="px-5 py-3 text-left cursor-pointer" @click="sortBy('origin')">
                    Origin
                    <SortIcon :active="sortKey === 'origin'" :asc="sortAsc" />
                </th>
                <th class="px-5 py-3 text-left cursor-pointer" @click="sortBy('destination')">
                    Destination
                    <SortIcon :active="sortKey === 'destination'" :asc="sortAsc" />
                </th>
                <th class="px-5 py-3 text-center cursor-pointer" @click="sortBy('date')">
                    Date
                    <SortIcon :active="sortKey === 'date'" :asc="sortAsc" />
                </th>
                <th class="px-5 py-3 text-center cursor-pointer" @click="sortBy('airline')">
                    Airline
                    <SortIcon :active="sortKey === 'airline'" :asc="sortAsc" />
                </th>
                <th class="px-5 py-3 text-center cursor-pointer" @click="sortBy('classType')">
                    Class Type
                    <SortIcon :active="sortKey === 'classType'" :asc="sortAsc" />
                </th>
                <th class="px-5 py-3 text-center cursor-pointer" @click="sortBy('passengers')">
                    Passengers
                    <SortIcon :active="sortKey === 'passengers'" :asc="sortAsc" />
                </th>
                <th class="px-5 py-3 text-right cursor-pointer" @click="sortBy('price')">
                    Price
        ...
        >
            ...
            <td class="px-5 py-3 text-center">{{ booking.airline }}</td>
            <td class="px-5 py-3 text-center">{{ booking.classType }}</td>
            <td class="px-5 py-3 text-center">{{ booking.passengers }}</td>
...
const fetchBookings = async () => {
  try {
    const response = await axios.get('http://localhost:5000/api/flightbookings')
...
const filteredBookings = computed(() => {
  ...
  return bookings.value.filter((b) => {
    const matchesSearch =
      ...
      b.airline?.toLowerCase().includes(s)
...
function exportBookings() {
  ...
  a.href = url
  a.download = 'flight_bookings_export.csv'
  a.click()
