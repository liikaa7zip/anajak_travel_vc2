<template>
  <div class="font-sans bg-gray-100 text-gray-900 min-h-screen">
    ...
    <section class="relative w-full h-80 sm:h-96">
      ...
      <div class="absolute inset-0 bg-black bg-opacity-40 flex items-center px-10">
        <h1 class="text-white text-3xl sm:text-6xl font-bold">
          <i class="fa-notdog fa-solid fa-location-dot"></i> {{ $t("Province.Kratie.KTHeroTitle") }} <br />
    ...
    <section class="pt-12 pb-6 pr-16 pl-16">
      <div class="bg-white rounded-lg shadow p-4 flex flex-col sm:flex-row items-start justify-between gap-4">
        <div class="max-w-xl m-8">
          <h2 class="text-2xl font-semibold mb-2 text-purple-600">{{ $t("Province.Kratie.aboutKT") }}</h2>
          <p class="text-lg text-gray-700">
            {{ $t("Province.Kratie.aboutKTDescription") }}
          </p>
        </div>
        <img
    ...
    <section class="pt-6 pb-6 pr-16 pl-16">
      <h2 class="text-2xl font-semibold mb-4 text-purple-600">{{ $t("Province.Kratie.Attractions.AttractionTitle") }}</h2>
    ...
    <section class="pt-6 pb-6 pr-16 pl-16 grid sm:grid-cols-2 gap-6">
      <div>
        <h2 class="text-2xl font-semibold mb-2 text-purple-600">{{ $t("Province.Kratie.ThingsToDo") }}</h2>
        <ul class="list-disc pl-5 text-lg text-gray-700">
          <li>{{ $t("Province.Kratie.First") }}</li>
          <li>{{ $t("Province.Kratie.Second") }}</li>
          <li>{{ $t("Province.Kratie.Third") }}</li>
        </ul>
      </div>
      <div>
        <h2 class="text-2xl font-semibold mb-2 text-purple-600">{{ $t("Province.Kratie.TravelTips") }}</h2>
        <ul class="list-disc pl-5 text-lg text-gray-700">
          <li>{{ $t("Province.Kratie.BestTime") }}</li>
          <li>{{ $t("Province.Kratie.Transport") }}</li>
          <li>{{ $t("Province.Kratie.DressCode") }}</li>
        </ul>
      </div>
    </section>
    ...
    <!-- Photo Gallery -->
    <section class="pt-6 pb-6 pr-16 pl-16">
      <h2 class="text-2xl font-semibold mb-4 text-purple-600">{{ $t("Province.Kratie.PhotoGallery") }}</h2>
      <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
    ...
    <!-- Province Map -->
    <section class="pt-6 pb-6 pr-16 pl-16">
      <h2 class="text-2xl font-semibold mb-4 text-purple-600">{{ $t("Province.Kratie.Map") }}</h2>
