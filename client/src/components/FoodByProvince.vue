    ...
    <div v-else class="flex-grow overflow-y-auto">
      ...
      >
        ...
        <div class="flex justify-end">
          ...
          <span
            v-else
            class="text-red-600 font-bold text-lg flex items-center space-x-2"
...
// import { useAuth } from "../stores/useAuth"; // Uncomment if you use an auth store
...
export default {
  ...
  data() {
    return {
      ...
      orders: [],
      // user: null, // Optional if you use auth
    };
  },
  watch: {
    locationId: {
  ...
  methods: {
    ...
    submitOrder() {
      // If you have auth, get user name from there. Otherwise ask for customerName or use a placeholder:
      // const auth = useAuth();
      // const user = auth.userProfile.value;
      // const customerName = user.fullName || user.username;

      const customerName = "Guest"; // Replace with actual user name or input from form
